    ...
    with open(vocab_file, 'r') as vocab_f:
      for line in vocab_f:
        ...
        if max_size != 0 and self._count >= max_size:
          print(
            "max_size of vocab was specified as %i; we now have %i words. Stopping reading." % (max_size, self._count))
          break
...
def abstract2sents(abstract):
  ...
  while True:
    try:
      start_p = abstract.index(bytes(SENTENCE_START, 'utf8'), cur)
      end_p = abstract.index(bytes(SENTENCE_END, 'utf8'), start_p + 1)
      cur = end_p + len(SENTENCE_END)
      sents.append(abstract[start_p+len(SENTENCE_START):end_p].decode('utf8'))
